    ...
    const NodeInfo<snowcrash::DataStructure>& dataStructure, ConversionContext& context)
{
    ...
    return std::make_unique<HolderElement>(SerializeKey::DataStructure, data::holder_t(std::move(msonElement)));
}
...
std::unique_ptr<IElement> MetadataToRefract(const NodeInfo<snowcrash::Metadata>& metadata, ConversionContext& context)
{
    ...
    element->meta().set(SerializeKey::Classes,
        generate_element<ArrayElement>([](auto& data) { //
            data.push_back(make_primitive(SerializeKey::User));
        }));

    element->get().key(make_primitive(metadata.node->first));
    element->get().value(make_primitive(metadata.node->second));
...
{
    auto element = CollectionToRefract<ArrayElement>(
        MAKE_NODE_INFO(parameter, values), context, LiteralToRefract, SerializeKey::Enum);

    // Add sample value
    if (!parameter.node->exampleValue.empty()) {
        auto samples = make_element<ArrayElement>();
        samples->get().push_back(
            CreateArrayElement(LiteralToRefract(MAKE_NODE_INFO(parameter, exampleValue), context)));
        element->attributes().set(SerializeKey::Samples, std::move(samples));
    }

    // Add default value
    if (!parameter.node->defaultValue.empty()) {
        element->attributes().set(SerializeKey::Default,
            CreateArrayElement(LiteralToRefract(MAKE_NODE_INFO(parameter, defaultValue), context)));
    }
...
std::unique_ptr<IElement> ExtractParameter(const NodeInfo<snowcrash::Parameter>& parameter, ConversionContext& context)
{
    ...
    if (parameter.node->values.empty()) {
        if (parameter.node->exampleValue.empty()) {
            element = make_element<StringElement>();
        } else {
            element = LiteralToRefract(MAKE_NODE_INFO(parameter, exampleValue), context);
        }
    ...
    const NodeInfo<snowcrash::Parameter>& parameter, ConversionContext& context)
{
    ...
    // Parameter use
    {
        ...
        element->attributes().set(SerializeKey::TypeAttributes, CreateArrayElement(make_primitive(use)));
    }
...
{
    auto element = make_element<MemberElement>(make_primitive(header.node->first), make_primitive(header.node->second));
    ...
    const NodeInfo<snowcrash::Asset>& asset, const std::string& contentType, const std::string& metaClass)
{
    ...
    element->meta().set(SerializeKey::Classes, CreateArrayElement(make_primitive(metaClass)));

    if (!contentType.empty()) {
        // FIXME: "contentType" has no sourceMap?
        element->attributes().set(SerializeKey::ContentType, make_primitive(contentType));
    }
...
std::unique_ptr<ArrayElement> ActionToRefract(const NodeInfo<snowcrash::Action>& action, ConversionContext& context)
{
    ...
    if (!action.node->relation.str.empty()) {
        ...
        auto relation = make_primitive(action.node->relation.str);
...
std::unique_ptr<ArrayElement> CategoryToRefract(const NodeInfo<snowcrash::Element>& element, ConversionContext& context)
{
    ...
    if (element.node->category == snowcrash::Element::ResourceGroupCategory) {
        category->meta().set(SerializeKey::Classes, CreateArrayElement(make_primitive(SerializeKey::ResourceGroup)));
        ...
        category->meta().set(SerializeKey::Classes, CreateArrayElement(make_primitive(SerializeKey::DataStructures)));
    }
    ...
    const NodeInfo<snowcrash::Blueprint>& blueprint, ConversionContext& context)
{
    ...
    ast->meta().set(SerializeKey::Classes, CreateArrayElement(make_primitive(SerializeKey::API)));
...
{
    auto element = make_primitive(annotation.message);

    element->element(SerializeKey::Annotation);

    element->meta().set(SerializeKey::Classes, CreateArrayElement(make_primitive(key)));

    element->attributes().set(SerializeKey::AnnotationCode, make_primitive(static_cast<double>(annotation.code)));
